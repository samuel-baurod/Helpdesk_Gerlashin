<?php
    ...
    switch($_GET["op"]){
        case "insert":
            $datos=$ticket->insert_ticket($_POST["usu_id"],$_POST["cat_id"],$_POST["subcat_id"],$_POST["tick_titulo"],$_POST["tick_descrip"],$_POST["priori_id"]);
        ...
        case "listar_x_usu":
            ...
            foreach($datos as $row){
                ...
                if ($row["tick_estado"]=="Abierto"){
                    ...
                    $sub_array[] ='<a onClick="CambiarEdoTicket('.$row["tick_id"].')"><span class="label label-pill label-danger">Cerrado</span></a>';
                }
                
                $sub_array[] = date("d/m/Y H:i:s", strtotime($row["fecha_creac"]));
        ...
        case "listar_filtro":
            $datos=$ticket->filtrar_ticket($_POST["tick_titulo"],$_POST["cat_id"],$_POST["priori_id"]);
            $data=Array();
            foreach($datos as $row){
                $sub_array = array();
                $sub_array[] = $row["tick_id"];
                        ...
                        ?>
                            <article class="activity-line-item box-typical">
                                <div class="activity-line-date">
                                    <?php echo date("d/m/Y", strtotime($row["fecha_creac"])); ?>
                                </div>
                                ...
                                <div class="activity-line-action-list">
                                    <section class="activity-line-action">
                                        <div class="time"><?php echo date("H:i:s", strtotime($row["fecha_creac"])); ?></div>
                                        <div class="cont">
                                            <div class="cont-in">
                                                <p>
        ...
        case "mostrar":
            ...
            if(is_array($datos)==true and count($datos)>0){
                foreach($datos as $row)
                {
                    ...
                    $output["fecha_creac"] = date("d/m/Y H:i:s", strtotime($row["fecha_creac"]));
                    $output["fecha_cierre"] = date("d/m/Y H:i:s", strtotime($row["fecha_cierre"]));
                    $output["usu_nom"] = $row["usu_nom"];
                    $output["usu_ape"] = $row["usu_ape"];
                    $output["cat_nom"] = $row["cat_nom"];
                    $output["subcat_nom"] = $row["subcat_nom"];
                    $output["tick_evalstar"] = $row["tick_evalstar"];
                    $output["tick_comentstar"] = $row["tick_comentstar"];
        ...
        case "insertdetalle":
            ...
            if(is_array($datos)==true and count($datos)>0){
                foreach($datos as $row){
                    ...
                    /* TODO: Consulta si vienen archivos desde la vista */
                    if(empty($_FILES['files']['name'])){
                        ...
                        /* TODO: Consultar si la ruta existe, en caso de que no, se creara */
                        if(!file_exists($ruta)){
                            mkdir($ruta, 0777, true);
                        }

                        /* TODO: Se recorren todos los registros */
                        for($index = 0; $index < $countfiles; $index++){
                            $doc1 = $_FILES['files']['tmp_name'][$index];
                            $destino = $ruta.$_FILES['files']['name'][$index];
        ...
        case "encuesta":
            $ticket->insert_encuesta($_POST["tick_id"],$_POST["tick_evalstar"],$_POST["tick_comentstar"]);
        break;
    }
?>